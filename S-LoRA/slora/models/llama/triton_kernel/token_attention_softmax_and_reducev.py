...
@triton.jit
def _fwd_kernel_softmax_reducev_gqa(
    Logics, V, Out,
    ...
    kv_heads: tl.constexpr,            # safety mask
    ...
    for start_n in range(0, cur_batch_seq_len, BLOCK_N):
        ...
        qk = tl.load(
            Logics + cur_q_head * stride_logic_h + (cur_batch_start_loc + start_n + offs_n) * stride_logic_bs,
            mask=start_n + offs_n < cur_batch_seq_len,
            other=float("-inf")
        )
        ...
        # If kv head is invalid (misconfig), treat V as zeros
        v = tl.load(
            v_ptrs + v_index[:, None] * stride_vbs,
            mask=(cur_kv_head < kv_heads),
            other=0.0
        )
    ...
    # Write output per Q head
    off_o = cur_batch * stride_obs + cur_q_head * stride_oh + offs_d * stride_od
    tl.store(Out + off_o, acc)


@torch.no_grad()
def token_softmax_reducev_fwd(
    logics, v, o, b_loc, b_start_loc, b_seq_len, max_input_len, other_kv_index, *, kv_heads: int
):
    """
    logics: [q_heads, total_token_num] (your layout)
    v: backing V cache tensor, typically [N, out_heads, d] (out_heads may be >= kv_heads)
    o: [B, q_heads, d]
    kv_heads: actual KV head count (e.g., 8)
    """
    BLOCK = 64
    batch = b_seq_len.shape[0]
    q_heads = logics.shape[0]
    ...
    _fwd_kernel_softmax_reducev_gqa[grid](
