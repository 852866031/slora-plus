...
class Llama3TransformerLayerInfer(LlamaTransformerLayerInfer):
    ...
    def _token_decode_attention_normal_alt(self, q, infer_state: LlamaInferStateInfo):
        """
        UnifiedMemoryAllocator (alt_mem_manager) decode attention path.
        Mirrors the parent logic but uses llama2 kernels and assumes they are GQA-correct.
        """
        total_token_num = infer_state.total_token_num
        batch_size = infer_state.batch_size
        calcu_shape1 = (batch_size, self.tp_q_head_num_, self.head_dim_)
        ...
        # Prepare layer-specific GPU buffer address and b_loc mappings
        buffer_address, gpu_b_loc_key, gpu_b_loc_value = infer_state.alt_mem_manager.prepare_b_locs_for_layer(
            infer_state.b_loc_key,
            infer_state.b_loc_value,
            infer_state.b_seq_len,
            self.layer_num_,
        )
