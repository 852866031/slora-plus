...
class UnifiedMemoryAllocator:
    ...
    def to_gpu_index(self, vpids, verbose=False) -> torch.Tensor:
        """
        ...
        if verbose: print(f"find cpu vpids time: {time.time()-start:.5f}")
        if cpu_vpids:
            start = time.time()
            self._pages_in(cpu_vpids)        # will evict if space is short
            if verbose: print(f"page in time: {time.time()-start:.5f}")
        start = time.time()
        self.assert_vpid_on_gpu(vpids)     # ensure all are on GPU
        if verbose: print(f"assert vpid time: {time.time()-start:.5f}")
        start = time.time()
        ...
        if verbose: print(f"to tensor time: {time.time()-start:.5f}")
        return gpu_idx
